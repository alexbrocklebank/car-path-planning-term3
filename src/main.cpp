      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			// If previous path size is almost empty, use the car as the starting reference
			if (prev_size < 2)
			{
			...
			// Otherwise use previous path's end point as starting reference
			else
			{
				...
				// Use two points to make path tangent to the previous paths end points
			...
			// Add evenly spaced 30m points in the Frenet S space
			for (int i = 1; i <= 3; i++)
			{
			...
			// Shift car reference angle to 0 for all points in pts_x and pts_y
